<?php
/**
 * Plugin Name: pg Payment Donation
 * Plugin URI: https://github.com/pg/
 * Description: This plugin allow you to accept donation payments using pg. This plugin will add a simple form that user will fill, when he clicks on submit he will redirected to pg website to complete his transaction and on completion his payment, pg will send that user back to your website along with transactions details. This plugin uses server-to-server verification to add additional security layer for validating transactions. Admin can also see all transaction details with payment status by going to "pg Payment Details" from menu in admin.
 * Version: 1.0
 * Author: pg
 * Author URI: http://pg.com/
 * Text Domain: pg Payments
 */

//ini_set('display_errors','On');
register_activation_hook(__FILE__, 'pg_activation');
register_deactivation_hook(__FILE__, 'pg_deactivation');

// do not conflict with WooCommerce pg Plugin Callback
if(!isset($_GET["wc-api"])){
	add_action('init', 'pg_donation_response');
}

add_shortcode( 'pgdonation', 'pg_donation_handler' );
...
if(isset($_GET['donation_msg']) && $_GET['donation_msg'] != ""){
	add_action('the_content', 'pgDonationShowMessage');
}

function pgDonationShowMessage($content){
...
function my_custom_fonts() {
  echo '<style>
   .toplevel_page_pg_options_page img{
      width:19px;
    } 
  </style>';
}
...
function pg_activation() {
	global $wpdb, $wp_rewrite;
	$settings = pg_settings_list();
	foreach ($settings as $setting) {
		add_option($setting['name'], $setting['value']);
	}
	add_option( 'pg_donation_details_url', '', '', 'yes' );
	$post_date = date( "Y-m-d H:i:s" );
	$post_date_gmt = gmdate( "Y-m-d H:i:s" );

	$ebs_pages = array(
		'pg-page' => array(
			'name' => 'pg Transaction Details page',
			'title' => 'pg Transaction Details page',
			'tag' => '[pg_donation_details]',
			'option' => 'pg_donation_details_url'
		),
	);
	
	$newpages = false;
	
	$pg_page_id = $wpdb->get_var("SELECT id FROM `" . $wpdb->posts . "` WHERE `post_content` LIKE '%" . $pg_pages['pg-page']['tag'] . "%'	AND `post_type` != 'revision'");
	if(empty($pg_page_id)){
		$pg_page_id = wp_insert_post( array(
			'post_title'	=>	$pg_pages['pg-page']['title'],
			'post_type'		=>	'page',
			'post_name'		=>	$pg_pages['pg-page']['name'],
			'comment_status'=> 'closed',
			'ping_status'	=>	'closed',
			'post_content' =>	$pg_pages['pg-page']['tag'],
			'post_status'	=>	'publish',
			'post_author'	=>	1,
			'menu_order'	=>	0
		));
	...
	update_option( $pg_pages['pg-page']['option'], _get_page_link($pg_page_id) );
	unset($pg_pages['pg-page']);
	
	$table_name = $wpdb->prefix . "pg_donation";
...
function pg_settings_list(){
	$settings = array(
		array(
			...
			'value'   => '',
			'type'    => 'textbox',
			'hint'    => 'Merchant Id Provided by pg'
		),
		array(
			'display' => 'Merchant Key',
			'name'    => 'pg_merchant_key',
			'value'   => '',
			'type'    => 'textbox',
			'hint'    => 'Merchant Secret Key Provided by pg'
		),
		array(
			'display' => 'Website',
			'name'    => 'pg_website',
			'value'   => '',
			'type'    => 'textbox',
			'hint'    => 'Website Name Provided by pg'
		),
		array(
			'display' => 'Industry Type',
			'name'    => 'pg_industry_type_id',
			'value'   => '',
			'type'    => 'textbox',
			'hint'    => 'Industry Type Provided by pg'
		),
		array(
			'display' => 'Channel ID',
			'name'    => 'pg_channel_id',
			'value'   => 'WEB',
			'type'    => 'textbox',
			'hint'    => 'Channel ID Provided by pg e.g. WEB/WAP'
		),
		array(
			...
			'value'   => '',
			'type'    => 'textbox',
			'hint'    => 'Transaction URL Provided by pg'
		),
		array(
			...
			'value'   => '',
			'type'    => 'textbox',
			'hint'    => 'Transaction Status URL Provided by pg'
		),
		array(
...
function pg_admin_menu() {
	add_menu_page('pg Donation', 'pg Donation', 'manage_options', 'pg_options_page', 'pg_options_page', plugin_dir_url(__FILE__).'assets/logo.ico');
	//, plugin_dir_url(__FILE__).'assets/logo.ico'

	add_submenu_page('pg_options_page', 'pg Donation Settings', 'Settings', 'manage_options', 'pg_options_page');

	add_submenu_page('pg_options_page', 'pg Donation Payment Details', 'Payment Details', 'manage_options', 'wp_pg_donation', 'wp_pg_donation_listings_page');
	
	require_once(dirname(__FILE__) . '/pg-donation-listings.php');
}


function pg_options_page() {
	echo	'<div class="wrap">
				<h1>pg Configuarations</h1>
			...
			$last_updated = "";
			$path = plugin_dir_path( __FILE__ ) . "/pg_version.txt";
			if(file_exists($path)){
				$handle = fopen($path, "r");
				if($handle !== false){
...
function pg_donation_handler(){

	if(isset($_REQUEST["action"]) && $_REQUEST["action"] == "pg_donation_request"){
		return pg_donation_submit();
	} else {
		return pg_donation_form();
	}
}

function pg_donation_form(){
	...
	$html = '<form name="frmTransaction" method="post">
					...
					<p>
						...
						<input type="text" name="donor_amount" maxlength="10" value="'.trim(get_option('pg_amount')).'" required />
					</p>
					<p>
					...
					<p>
						<input type="hidden" name="action" value="pg_donation_request">
						<input type="submit" value="' . trim(get_option('pg_content')) .'" required />
					</p>
				</form>';
...
function pg_donation_submit(){
	...
	if($valid){
		...
		//End Post Request
       //echo get_option('pg_merchant_id');exit;
 		$post_params = array(
			'PAY_ID' => trim(get_option('pg_merchant_id')),
				'ORDER_ID' =>$order_id,
		...
		$post_params["HASH"] = PayDonation::getHashFromArray(	$post_params,
																				trim(get_option('pg_merchant_key'))
																			);
...
function pg_donation_meta_box() {
	$screens = array( 'pgdonation' );
	
	foreach ( $screens as $screen ) {
		add_meta_box(  'myplugin_sectionid', __( 'pg', 'myplugin_textdomain' ),'pg_donation_meta_box_callback', $screen, 'normal','high' );
	}
}

add_action( 'add_meta_boxes', 'pg_donation_meta_box' );

function pg_donation_meta_box_callback($post) {
	echo "admin";
}

function pg_donation_response(){
	global $wpdb;
	...
	if($_POST['STATUS']=="Captured")
{
		...
		$pg_merchant_key = trim(get_option('pg_merchant_key'));
		$pg_merchant_id = trim(get_option('pg_merchant_id'));
   ...
   $msg = "Thank you for your order. Your transaction has been successful.";
					$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix . "pg_donation SET payment_status = 'Complete Payment' WHERE  id =%d", sanitize_text_field($_POST['ORDER_ID'])));
				
				}
// 				else{

// $msg = "It seems some issue in server to server communication. Kindly connect with administrator.";
// 					$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix . "pg_donation SET payment_status = 'Fraud Payment' WHERE id = %d", sanitize_text_field($_POST['ORDER_ID'])));
// 				}
		...
		// if(pgDonation::verifychecksum_e($_POST, $pg_merchant_key, $_POST['CHECKSUMHASH']) === "TRUE") {
			
		// 	if($_POST['RESPCODE'] == "01"){

		// 		// Create an array having all required parameters for status query.
		// 		$requestParamList = array("MID" => $pg_merchant_id, "ORDERID" => $_POST['ORDERID']);

		// 		// $_POST['ORDERID'] = substr($_POST['ORDERID'], strpos($_POST['ORDERID'], "-") + 1); // just for testing
				
		// 		$StatusCheckSum = pgDonation::getChecksumFromArray($requestParamList, $pg_merchant_key);

		// 		$requestParamList['CHECKSUMHASH'] = $StatusCheckSum;

		// 		$responseParamList = pgDonation::callNewAPI($transaction_status_url, $requestParamList);

		// 		if($responseParamList['STATUS'] == 'TXN_SUCCESS' && $responseParamList['TXNAMOUNT'] == $_POST['TXNAMOUNT']) {
		// 			$msg = "Thank you for your order. Your transaction has been successful.";
		// 			$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix . "pg_donation SET payment_status = 'Complete Payment' WHERE  id = %d", sanitize_text_field($_POST['ORDERID'])));
				
		// 		} else  {
		// 			$msg = "It seems some issue in server to server communication. Kindly connect with administrator.";
		// 			$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix . "pg_donation SET payment_status = 'Fraud Payment' WHERE id = %d", sanitize_text_field($_POST['ORDERID'])));
		// 		}

		// 	} else {
		// 		$msg = "Thank You. However, the transaction has been Failed For Reason: " . sanitize_text_field($_POST['RESPMSG']);
		// 		$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix . "pg_donation SET payment_status = 'Cancelled Payment' WHERE id = %d", sanitize_text_field($_POST['ORDERID'])));
		// 	}
		// } else {
		// 	$msg = "Security error!";
		// 	$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix . "pg_donation SET payment_status = 'Payment Error' WHERE  id = %d", sanitize_text_field($_POST['ORDERID'])));
		// }
...
* Code to test Curl
*/
if(isset($_GET['pg_action']) && $_GET['pg_action'] == "curltest"){
...
function curltest_donation($content){
	...
	$content = "<center><h1>cURL Test for pg Donation Plugin</h1></center><hr/>";
